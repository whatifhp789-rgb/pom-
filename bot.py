...
4. DYNAMIC UPI QR GENERATOR — Auto generates QR with plan amount.
5. MULTI-OWNER SUPPORT — 2 owners can manage the bot.
"""
...
DEFAULTS = {
    ...
    "admin_chat_id": "8754004223",
    "welcome_text": "Welcome! Choose an option below.",
    "welcome_photo": "",
    "access_link": "",
    "upi_id": "your-upi@paytm",  # 🔥 Default UPI ID
...
def media_list(scope):
    with db() as c:
        return [dict(r) for r in c.execute("SELECT * FROM media WHERE scope = ? ORDER BY position, id", (scope,)).fetchall()]
...
def set_step(chat_id, step):
    with db() as c:
        if step:
            c.execute("INSERT INTO state (chat_id, step) VALUES (?, ?) ON CONFLICT(chat_id) DO UPDATE SET step = excluded.step",
                      (str(chat_id), step))
        else:
...
def stats():
    with db() as c:
        rows = c.execute("SELECT status, COUNT(*) AS n, COALESCE(SUM(price), 0) AS total FROM payments GROUP BY status").fetchall()
...
def new_order_code():
    with db() as c:
        for _ in range(50):
            ...
            row = c.execute("SELECT 1 FROM payments WHERE order_code = ?", (code,)).fetchone()
            if not row:
                return code
...
def send_photo(chat_id, file_id, caption="", keyboard=None):
    ...
    if file_id.startswith("http") or os.path.exists(file_id):
        # Local file or URL
        if os.path.exists(file_id):
            with open(file_id, 'rb') as f:
                return call("sendPhoto", chat_id=chat_id, photo=f, caption=caption[:1024], parse_mode="HTML")
        else:
            return call("sendPhoto", chat_id=chat_id, photo=file_id, caption=caption[:1024], parse_mode="HTML")
...
def dashboard_text():
    s = stats()
    with db() as c:
        rows = c.execute("SELECT * FROM payments WHERE status = 'pending' ORDER BY id DESC LIMIT 5").fetchall()
    lines = [
        "📊 <b>Dashboard</b>",
        "",
    ...
    return {
        "inline_keyboard": [
            ...
            [{"text": "📢 Broadcast", "callback_data": "bcast"},
             {"text": "💳 Set UPI", "callback_data": "set:upi_id"}],
        ]
    }
...
def handle_message(msg):
    ...
    if step == "report" and not (text.lower().startswith("/")):
        ...
        header = (f"🚨 <b>Issue reported</b>\nFrom: {who} (<code>{chat_id}</code>)" + (f"\n\n{text}" if text else ""))
        admin = get("admin_chat_id")
        if admin:
            if file_id and kind == "photo":
                send_photo(admin, file_id, header)
            elif file_id:
                call("sendVideo", chat_id=admin, video=file_id, caption=header[:1024], parse_mode="HTML")
            else:
                send(admin, header)
        return send(chat_id, "✅ Thanks! Your issue has been sent to the admin. We'll get back to you as soon as possible.",
                    start_keyboard())
            ...
            for cid in targets:
                try:
                    if file_id and kind == "photo":
                        ...
                        ok = call("sendVideo", chat_id=cid, video=file_id, caption=text[:1024], parse_mode="HTML")
                    else:
                        ok = send(cid, text)
                    if ok and ok.get("ok"):
                        sent += 1
                    ...
                    failed += 1
                time.sleep(0.05)
            return send(chat_id, f"📢 Broadcast done.\nSent: <b>{sent}</b>\nFailed: <b>{failed}</b>", dashboard_keyboard())
    ...
    if file_id and kind == "photo" and not is_admin(chat_id):
        with db() as c:
            if group_id:
                dup = c.execute("SELECT 1 FROM payments WHERE chat_id = ? AND media_group_id = ?",
                                (str(chat_id), group_id)).fetchone()
                if dup:
                    return None
            sel = c.execute("SELECT * FROM payments WHERE chat_id = ? AND status = 'selected' ORDER BY id DESC LIMIT 1",
                            (str(chat_id),)).fetchone()
            if sel:
                pid = sel["id"]
                c.execute("UPDATE payments SET photo_file_id = ?, media_group_id = ?, status = 'pending' WHERE id = ?",
                          (file_id, group_id, pid))
                label, price, code = sel["plan_label"], sel["price"], sel["order_code"]
            else:
                code = new_order_code()
                cur = c.execute(
                    "INSERT INTO payments (order_code, chat_id, username, full_name, plan_label, price, photo_file_id, media_group_id, status, created_at) VALUES (?,?,?,?,?,?,?,?,'pending',?)",
...
def handle_callback(cq):
    ...
    if data.startswith("plan:"):
        ...
        # 2) Generate dynamic QR with amount
        code = new_order_code()
        ...
        with db() as c:
            ...
            c.execute(
                "INSERT INTO payments (order_code, chat_id, username, full_name, plan_label, price, status, created_at) VALUES (?,?,?,?,?,?, 'selected', ?)",
                (code, str(chat_id), frm.get("username", ""), frm.get("first_name", ""),
        ...
        qr_path = generate_upi_qr(upi_id, p["price"], code)
        ...
        if qr_path and os.path.exists(qr_path):
            ...
            if qr:
                ...
                send(chat_id, "QR is not set yet. Please contact support.")
        return
    ...
    if data == "bcast":
        ...
        answer()
        return send(chat_id, f"📢 Send the message (text, photo or video) to broadcast to all {len(all_chat_ids())} users.",
    ...
    if data.startswith("madd:"):
        ...
        answer()
        return send(chat_id, f"Send photos/videos one by one (up to {MEDIA_LIMIT}). Tap Done when finished.",
                    {"inline_keyboard": [[{"text": "✔️ Done", "callback_data": f"mdone:{scope}"}]]})
    ...
    if data.startswith("set:"):
        ...
        if key == "upi_id":
            return send(chat_id, "Send your UPI ID (e.g., your-upi@paytm)")
    ...
    if data == "pay:list":
        answer()
        with db() as c:
            rows = c.execute("SELECT * FROM payments WHERE status = 'pending' ORDER BY id DESC LIMIT 10").fetchall()
        if not rows:
...
def first_run_setup():
    ...
    if not get("admin_chat_id"):
        print("Send /admin to your bot from your own account, then paste the chat id it replies with. Leave empty to skip for now.")
    ...
    while True:
        try:
            ...
            if res.get("error_code") == 409:
                print("[conflict] Another instance is polling this bot token. Stop it.")
                time.sleep(5)
                continue
            ...
            if updates:
                ...
                try:
                    requests.get(f"{API}/bot{get('bot_token')}/getUpdates",
